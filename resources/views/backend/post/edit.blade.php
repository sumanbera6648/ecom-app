...
@section('sub_breadcrumb', 'Edit Posts')
@section('main-content')
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-md-12 layout-spacing">
            <div class="card">
                <h5 class="mt-5 mx-3">Add Post</h5>
                <div class="card-body">
                    <form method="post" action="{{ route('post.update', $post->id) }}" enctype="multipart/form-data">
                            ...
                            <select name="post_cat_id" class="form-control">
                                ...
                                @foreach ($categories as $key => $data)
                                    <option value='{{ $data->id }}'
                                        {{ $data->id == $post->post_cat_id ? 'selected' : '' }}>{{ $data->title }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @endphp
                        <div class="form-group">
                            <label for="tags">Tag <span class="text-danger">*</span></label>
                            <select name="tags[]" class="form-control">
                                <option value="">--Select any tag--</option>
                                @foreach ($tags as $key => $data)
                                    <option value="{{ $data->title }}"
                                        {{ in_array("$data->title", $post_tags) ? 'selected' : '' }}>
                                        {{ $data->title }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="added_by" class="form-control">
                                ...
                                @foreach ($users as $key => $data)
                                    <option value='{{ $data->id }}'
                                        {{ $post->added_by == $data->id ? 'selected' : '' }}>{{ $data->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" value="{{ $post->status }}" class="form-control">
